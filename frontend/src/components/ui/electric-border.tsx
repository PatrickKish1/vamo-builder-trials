...
const ElectricBorder: React.FC<ElectricBorderProps> = ({
  ...
  // Use separate colors for top and bottom if provided, otherwise use single color
  const topBorderColor = topColor || color;
  const bottomBorderColor = bottomColor || color;
  ...
  // Create gradient border effect with top and bottom colors
  const strokeStyle: CSSProperties = {
    ...inheritRadius,
    borderWidth: thickness,
    borderStyle: 'solid',
    borderColor: topBorderColor, // Fallback
  };
  ...
  const glow1Style: CSSProperties = {
    ...
    borderWidth: thickness,
    borderStyle: 'solid',
    borderColor: hexToRgba(topBorderColor, 0.6),
  ...
  const bgGlowStyle: CSSProperties = {
    ...
    opacity: 0.3,
    zIndex: -1,
    background: `linear-gradient(-30deg, ${hexToRgba(topBorderColor, 0.8)}, transparent, ${bottomBorderColor})`
  };
  ...
  return (
    <div ref={rootRef} className={'relative isolate ' + (className ?? '')} style={style}>
      ...
      <div className="absolute inset-0 pointer-events-none" style={inheritRadius}>
        {/* Gradient border using pseudo-elements with CSS gradients */}
        <div 
          ref={strokeRef}
          ...
          style={{
            ...
            borderWidth: thickness,
            borderStyle: 'solid',
            borderImage: `linear-gradient(to bottom, ${topBorderColor}, ${bottomBorderColor}) 1`,
            filter: `url(#${filterId})`,
          }}
        />
